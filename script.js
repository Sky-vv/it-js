...
console.log(
  s1,
  'см2 площадь прямоугольника - второй вариант выводы.',
  'Задание 4'
);
...
function GenerateResultForIpoteka(price, percent, years) {
  let per = percent / 100 / 12;
  let monthN = years * 12;
  let r =
    price *
    ((per * Math.pow(1 + per, monthN)) / (Math.pow(1 + per, monthN) - 1));
  return r.toFixed(2);
}
...
let price = 2000000,
  percent = 10,
  years = 5;
let amount = GenerateResultForIpoteka(price, percent, years) * (years * 12);
let perepl = GenerateResultForIpoteka(price, percent, years) * (years * 12) - price;

console.log('Общая сумма кредита: ' + price + ' руб');
console.log('Ставка: ' + percent + ' %');
console.log('Срок кредита: ' + years + ' лет');
console.log('Ежемесячный платеж: ' + GenerateResultForIpoteka(price, percent, years) + ' руб');
